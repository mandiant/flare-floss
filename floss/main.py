...
from floss.const import MAX_FILE_SIZE, MEGABYTE, MIN_STRING_LENGTH, SUPPORTED_FILE_MAGIC
...
def make_parser(argv):
    ...
    advanced_group.add_argument(
        ...
        "--load",
        action="store_true",
        help="load from existing FLOSS results document"
        if show_all_options
        else argparse.SUPPRESS,
    )
    advanced_group.add_argument(
    ...
    advanced_group.add_argument(
        ...
        "--large-file",
        action="store_true",
        help="allow processing files larger than {} MB".format(
        ...
        if results.analysis.enable_decoded_strings:
            ...
            if len(fvas_to_emulate) == 0:
                ...
                for fva in fvas_to_emulate:
                    ...
                    logger.debug(
                        "  - 0x%x: score: %.3f, xrefs_to: %d", fva, score, xrefs_to
                    )
