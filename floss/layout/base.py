...
class Layout(BaseModel, abc.ABC):
    ...
    def extract_strings(self, min_len: int, strings: Optional[List[ExtractedString]] = None) -> None:
        ...
        self.strings = []
        child_strings = {id(child): [] for child in self.children}
        ...
        for s in strings:
            ...
            # This follows the Bug 1 fix strategy of overlaying metadata by intersecting start offset
            # with node boundaries.
            start = s.slice.range.offset
            matched_child = None
            for child_start, child_end, child in child_ranges:
                if child_start <= start < child_end:
                    matched_child = child
                    break
