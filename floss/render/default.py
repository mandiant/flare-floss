...
def render_language_strings(
    ...
    for s in strings:
        if verbose == Verbosity.DEFAULT:
            ...
            # --- NEW LOGIC START ---
            # If the string has an address (like our Go strings now do), format it.
            # Otherwise, leave it blank (for normal static strings).
            va_str = f" (0x{s.address:x})" if getattr(s, "address", 0) else ""

            # Print the offset, then the VA, then the string
            console.print(f"0x{s.offset:>0{offset_len}x}{va_str} {colored_string}")
            # --- NEW LOGIC END ---
