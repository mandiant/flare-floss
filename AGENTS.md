...
- To setup the development environment, make sure that a venv is created and the pre-commit and pre-push hooks are installed.
- All lints, formatters and tests **must** pass before making a PR. Enforce this strictly.
- The `floss/` folder has the main functionality, while `scripts/` has auxillary plugins and scripts. Docs are in `doc/`.
